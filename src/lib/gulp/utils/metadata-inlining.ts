...
export function inlinePackageMetadataFiles(packagePath) {
  ...
  glob.sync(join(packagePath, '**/*.+(html|css)')).forEach(resourcePath => {
  ...
  glob.sync(join(packagePath, '**/*.metadata.json')).forEach(path => {
