 ...
 */
export function getSecondaryEntryPointsForPackage(pkg: PackageModel) {
  const packageName = pkg.name;
  const namespace = pkg.namespace;
  const packageDir = pkg.config.src;
  ...
  const buildNodes = entryPoints.map(p => ({name: p, deps: [], depth: 0}));
  ...
  buildNodes.forEach(node => {
    const importStatementFindCommand = buildPackageImportStatementFindCommand(
      join(packageDir, node.name), namespace, packageName);
  ...
  const buildOrder = buildNodes.reduce((order, node) => {
 ...
 */
function partitionNodesByDepth(nodes) {
  const result = [[]];
  let lastDepth = 0;
...
function buildPackageImportStatementFindCommand(searchDirectory, namespace, packageName) {
  if (platform() === 'win32') {
    return {
      binary: 'findstr',
